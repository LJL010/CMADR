# LM.py - 修正版本：严格按照论文实现拉格朗日方法，修复数据类型错误
...
def train_cmadr(env, mac, num_episodes=500, gamma=0.98, cost_limits=None, device='cpu', batch_size=50):
    ...
    for ep in range(num_episodes):
        ...
        for start in range(0, T, batch_size):
            ...
            act_batch = torch.from_numpy(np.array(actions_list[start:end], dtype=np.int64)).to(device)  # 修复：确保为int64
            ...
            for agent_idx in range(n_agents):
                ...
                act_agent = act_batch[:, agent_idx]  # 这里已经是int64类型
                ...
                if B > 1:
                    ...
                    # 修复：确保act_agent[:-1]是int64类型
                    act_agent_indices = act_agent[:-1].long()  # 确保转换为int64
                    selected_probs = probs.gather(1, act_agent_indices.unsqueeze(-1)).squeeze(-1)
